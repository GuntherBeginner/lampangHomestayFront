...
export default function BookingUpdate() {
    ...
    useEffect(() => {
        if (B_id) {
            fetch(URL + '/booking/' + B_id)
                .then(res => res.json())
                .then(
                    (result) => {
                        setState({
                            ...result[0],
                            D_from: dayjs(result[0].D_from).format(dateFormat),
                            D_to: dayjs(result[0].D_to).format(dateFormat),
                        })
                    }
                )
        }
    ...
    const handleSubmit = event => {
        ...
        fetch(URL + '/updateBooking', {
            ...
            .then(
                (result) => {
                    console.log(result);
                    alert('Edit Booking : ' + B_id + ' Success!!')
